...
func NewUDP(redisWrapper *config.RedisWrapper) *UDP {
	return &UDP{redisWrapper: redisWrapper}
}
...
func printUDP(analysis *model.PacketAnalysisUDP) {
	...
	if analysis.IPv4 != nil {
		fmt.Printf("Version: %d | IHL: %d bytes | Total Length: %d | TTL: %d | Protocol: %d\n",
		...
		fmt.Printf("Version: %d | Payload Length: %d | Hop Limit: %d | Next Header: %d\n",
	...
	if analysis.UDP.SourcePort != 0 && analysis.UDP.DestinationPort != 0 {
		fmt.Printf("%s:%d → %s:%d | Length: %d | Checksum: 0x%X\n",
			analysis.IPv4.SourceIP, analysis.UDP.SourcePort,
